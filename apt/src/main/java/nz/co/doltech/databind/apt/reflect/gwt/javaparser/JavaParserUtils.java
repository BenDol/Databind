...
public final class JavaParserUtils {
     ...
     * <p>
     * You should use {@link #getJavaType(CompilationUnitService, Type, Set)}
    ...
    public static JavaType getJavaType(
        ...
        final JavaPackage compilationUnitPackage = compilationUnitService
                .getUnitPackage();
        ...
        if (nameToFind instanceof QualifiedNameExpr) {
            ...
            final NameExpr enclosedBy = getNameExpr(compilationUnitService
            ...
            if (isEqual(qneQualifier, enclosedBy)) {
                ...
                // reference
                final String name = compilationUnitService
                        .getEnclosingTypeName().getFullyQualifiedTypeName()
                        + "." + nameToFind.getName();
                return new JavaType(name,
                        compilationUnitService.getEnclosingTypeName());
            }
            ...
            if (qne.toString().length() > 1
                ...
                final ImportDeclaration importDeclaration = getImportDeclarationFor(
                    compilationUnitService, qne.getQualifier());
                if (importDeclaration == null) {
                    if (!compilationUnitPackage.getFullyQualifiedPackageName()
                            .equals("")) {
                        // It was not imported, so let's assume it's in the same
                        // package
                        return new JavaType(compilationUnitService.getUnitPackage()
                            .getFullyQualifiedPackageName() + "." + qne.toString());
                    }
                }
                else {
        ...
        if ("?".equals(nameToFind.getName())) {
            return new JavaType(JavaType.OBJECT.getFullyQualifiedTypeName(), 0,
        ...
        final NameExpr enclosingTypeName = getNameExpr(compilationUnitService
                .getEnclosingTypeName().getSimpleTypeName());
        if (isEqual(enclosingTypeName, nameToFind)) {
            return compilationUnitService.getEnclosingTypeName();
        }
        ...
        for (final TypeDeclaration internalType : compilationUnitService
                .getInnerTypes()) {
        ...
        final ImportDeclaration importDeclaration = getImportDeclarationFor(
            compilationUnitService, nameToFind);
        if (importDeclaration == null) {
     ...
     * primitives.
     * 
     * @param compilationUnitService to use for package resolution (required)
    ...
    public static JavaType getJavaType(
        ...
        if (internalType instanceof WildcardType) {
            ...
            if (wt.getSuper() != null) {
                ...
                final JavaType effectiveType = getJavaTypeNow(
                    compilationUnitService, cit, typeParameters);
        ...
        final JavaType effectiveType = getJavaTypeNow(compilationUnitService,
                cit, typeParameters);
        if (array > 0) {
            return new JavaType(effectiveType.getFullyQualifiedTypeName(),
    ...
    public static JavaType getJavaTypeNow(
        ...
        if (cit.getTypeArgs() != null) {
            for (final Type ta : cit.getTypeArgs()) {
                parameterTypes.add(getJavaType(compilationUnitService, ta,
                        typeParameters));
            }
        }
    ...
    public static Type getResolvedName(final JavaType target,
            ...
            final CompilationUnitService compilationUnitService) {
        final NameExpr nameExpr = JavaParserUtils.importTypeIfRequired(target,
                compilationUnitService.getImports(), current);
        final ClassOrInterfaceType resolvedName = JavaParserUtils
                .getClassOrInterfaceType(nameExpr);
        if (current.getParameters() != null
                && current.getParameters().size() > 0) {
            ...
            for (final JavaType param : current.getParameters()) {
                resolvedName.getTypeArgs().add(
                        getResolvedName(target, param, compilationUnitService));
            }
        }
