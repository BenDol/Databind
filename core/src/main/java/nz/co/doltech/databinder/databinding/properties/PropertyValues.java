...
import nz.co.doltech.databinder.databinding.PlatformSpecific;
import nz.co.doltech.databinder.databinding.PlatformSpecificProvider;
import nz.co.doltech.databinder.databinding.propertyadapters.CompositePropertyAdapter;
import nz.co.doltech.databinder.databinding.tools.Property;
...
class PropertyValues {
     ...
     */
    void setObjectDynamicProperty(Object object, String propertyName,
                                  Object value) {
        DynamicPropertyBag bag = propertyBagAccess
            .getObjectDynamicPropertyBag(object);
        if (bag == null) {
    ...
    private <T> T getPropertyImpl(Object object, String name) {
        ...
        if (name.equals(CompositePropertyAdapter.DTOMAP_TOKEN))
            throw new RuntimeException(
                "Property of type $DTOMap cannot be readden !");

        // if has dynamic-property, return it !
        if (hasObjectDynamicProperty(object, name)) {
            LOGGER.fine("'" + name + "' read dynamic property on object "
                + object);
        ...
        if (getter != null) {
            try {
                ...
                throw new RuntimeException("ObjectAdapter [object]."
                    + object.getClass().getName() + "." + getterName
                    + "() : getter call throwed an exception. See cause.",
                    e);
            }
        }
    ...
    private boolean setPropertyImpl(Clazz<?> s, Object object, String name,
                                    Object value) {
        if (PlatformSpecificProvider.get().isBindingToken(name))
            return PlatformSpecificProvider.get().setBindingValue(object, name,
                value);
        ...
        if (!hasObjectDynamicProperty(object, name))
            LOGGER.warning("'"
                + name
                + "' write dynamic property on object "
                + object.getClass().getName()
                + " with value "
                + value
                + " WARNING : THAT MEANS THERE IS NO GETTER/SETTER/FIELD FOR THAT CLASS ! PLEASE CHECK THAT IT IS REALLY INTENTIONAL !");
